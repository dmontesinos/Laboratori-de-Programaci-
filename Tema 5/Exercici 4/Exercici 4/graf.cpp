...
{
	vector<string>::iterator itNode = find(m_nodes.begin(), m_nodes.end(), nodeInicial);
	vector<string>::iterator itNodeFinal = find(m_nodes.begin(), m_nodes.end(), nodeFinal);

	vector<int> v_nomValor(m_numNodes, -1);
	vector<bool> visitat;
	visitat.resize(m_numNodes, false);
	queue<pair<int, int>> pendents;
	vector<int> nivells(m_numNodes + 1, -1);
	int posicion;
	bool mostrar = false;

	posicion = distance(m_nodes.begin(), itNodeFinal);

	if (std::find(m_nodes.begin(), m_nodes.end(), nodeFinal) != m_nodes.end())
	{
		mostrar = true;
	}
	...
	if (itNode != m_nodes.end())
	{
		int pos = distance(m_nodes.begin(), itNode);
		visitat[pos] = true;
		pendents.push(make_pair(pos, 0));

		while (!pendents.empty())
		{
			pair<int, int> nodeActual = pendents.front();
			pendents.pop();
			recorregut.push(m_nodes[nodeActual.first]);

			// posem a la cua tots els nodes adjacents a nodeActual no visitats
			for (int col = 0; col < m_numNodes; col++)
			{
				if ((m_matriuAdj[nodeActual.first][col] != 0) && (!visitat[col]))
				{
					v_nomValor[col] = nodeActual.first;
					pendents.push(make_pair(col, nodeActual.second + 1));
					visitat[col] = true;
				}
			}
		}
	...
	if (mostrar)
	{
		while (posicion != -1)
		{
			cami.push(m_nodes[posicion]);
			posicion = v_nomValor[posicion];
		}
	}
}
...
int Graf::BFS(string nodeInicial, string nodeFinal, queue<string>& recorregut)
{
	...
	visitat.resize(m_numNodes, false);
	queue<pair<int,int>> pendents;
	vector<int> nivells(m_numNodes+1, 0);
	int posicion = 0;
		...
		while (!pendents.empty())
		{
			...
			for (int col = 0; col < m_numNodes; col++)
			{
				if ((m_matriuAdj[nodeActual.first][col] != 0) && (!visitat[col]))
				{
					...
					nivells[col] = nivells[nodeActual.first] + 1;
				}
			}
			//nivells[posicion] = nodeActual.second;
		}
	}
	if (nivells[posicion] == 0)
		return -1;
	else
		return nivells[posicion];
	//int pos = distance(m_nodes.begin(), itNodeFinal);
	//return nivells[pos];
}
