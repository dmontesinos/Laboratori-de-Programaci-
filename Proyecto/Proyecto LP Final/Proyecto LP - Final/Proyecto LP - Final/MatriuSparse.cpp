...
{
	vMaps.resize(m_coordenadas.size());

	for (auto it = vMaps.begin(); it != vMaps.end(); it++)
	{
		int posicion = distance(vMaps.begin(), it);
		(*it)[make_pair(get<0>(m_coordenadas[posicion]), get<1>(m_coordenadas[posicion]) )] = 0;
	}
}

void MatriuSparse::calculaDendograms(vector<Tree<double>*>& vDendograms) const
{
	/*Tree<double> *elemento;
	vector<Tree<double>*> endogramas;
	endogramas.resize(m_dimension);

	for (int i = 0; i < endogramas.size(); i++)
	{
		endogramas[i] = elemento(i);
	}*/
}
